...
포트를 하나씩 확인하는 방식으로 시작했다. 먼저 22번 포트를 대상으로 서비스 버전을 확인했다.
...
버전 확인은 가능했지만 플래그는 없다. 53번 포트도 확인해봤다.
...
`-sA` ACK 스캔과 `--source-port=53`을 조합해 상위 30개 포트의 방화벽 상태를 분석했다.
...
`-sA` 결과만으로는 실제 오픈 포트를 알 수 없다. 디코이(`-D RND:10`)를 붙여 SYN 스캔으로 상위 10개 포트를 확인했다.
...
오픈 포트: **22, 80, 110, 139, 445**. 이어서 버전 스캔을 실행했다.
...
전체 포트 스캔(`-p-`)을 ACK 스캔으로 시도했더니 IDS 알림이 증가하고 스캔 속도가 급격히 느려졌다.
...
스캔할 때마다 알림이 2씩 증가한다. 75개 제한이 있으므로 스캔 횟수를 아껴야 한다.
...
디코이 100개를 붙인 SYN + 버전 전체 포트 스캔을 실행했다.
...
방화벽이 소스포트 53(DNS)에서 오는 트래픽을 신뢰하도록 설정되어 있을 가능성이 높다. `ncat`으로 소스포트를 53으로 지정해 포트 50000에 직접 연결했다.
